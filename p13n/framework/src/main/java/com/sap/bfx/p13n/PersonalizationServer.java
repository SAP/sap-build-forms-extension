...
@GrpcService @Slf4j public class PersonalizationServer extends P13nServiceGrpc.P13nServiceImplBase {
    ...
    @Override public void getSettings(GetSettingsRequest request,
        ...
        for (var element : result) {
            settings.add(Settings.newBuilder().setId(element.getId().toString()).setUser(element.getUser())
                                 .setKey(element.getKey()).setApp(element.getApp()).setEncoding(element.getEncoding())
     ...
     * @param responseObserver - response observer
     */
    public void changeSettings(ChangeSettingsRequest request, StreamObserver<ChangeSettingsResponse> responseObserver) {
        Collection<Settings> settings = new ArrayList<>();
        ...
        for (Settings setting : request.getSettingsList()) {
            ...
            if (resultOpt.isEmpty()) {
                ...
                Personalization lastSuitablePersonalization =
                        service.findPersonalizationByKeyUserAndApp(p.getKey(), p.getUser(), p.getApp());
                ...
                if (lastSuitablePersonalization != null &&
                        ...
                        Objects.equals(lastSuitablePersonalization.getApp(), p.getApp())) {
                    continue;
                }
                ...
                if (lastSuitablePersonalization == null ||
                    ...
                    settings.add(
                            Settings.newBuilder().setId(p.getId().toString()).setUser(p.getUser()).setKey(p.getKey())
                                    .setApp(p.getApp()).setEncoding(p.getEncoding()).setValue(p.getValue()).build());
                }
                // update personalization
            } else if (resultOpt.get() != p && resultOpt.get().isEditable() && resultOpt.get().isVisible()) {
                service.updatePersonalizationUser(p);
                settings.add(Settings.newBuilder().setId(p.getId().toString()).setUser(p.getUser()).setKey(p.getKey())
                                     .setApp(p.getApp()).setEncoding(p.getEncoding()).setValue(p.getValue()).build());
            }
        }
     ...
     * @param responseObserver - response observer
     */
    public void deleteSettings(DeleteSettingsRequest request, StreamObserver<DeleteSettingsResponse> responseObserver) {
        Collection<Settings> settings = new ArrayList<>();
