...
public class ValueHelpDaoHana extends AbstractValueHelpDao {
    ...
    public Collection<String> findAllAdapter() {
        return jdbc.queryForList("SELECT DISTINCT adapter FROM forms_valuehelp.forms_vh_defs", String.class);
    }
    ...
    public void addValue(String id, Long version, String locale, java.sql.Timestamp validUntil, String values) {
        jdbc.update(con -> {
            PreparedStatement ps = con.prepareStatement(
                    "INSERT INTO forms_valuehelp.forms_vh_values (id,version,locale,valid_until,\"values\") VALUES (?,?,?,?,?)");
            ps.setString(1, id);
    ...
    public void updateValue(String id, Long version, String locale, java.sql.Timestamp validUntil, String values) {
        jdbc.update(con -> {
            PreparedStatement ps = con.prepareStatement(
                    "UPDATE forms_valuehelp.forms_vh_values SET \"values\"=?,valid_until=?, version=? WHERE id=? and locale=?");
    ...
    public void deleteValue(String id, String locale, long version) {
        jdbc.update(con -> {
            PreparedStatement ps = con.prepareStatement(
                    "DELETE FROM forms_valuehelp.forms_vh_values WHERE id = ? AND locale = ? AND version = ?");
            ps.setString(1, id);
            ps.setString(2, locale);
    ...
    public Pair<String, Long> findById(String id, String locale) {
        ...
        jdbc.query("SELECT version,\"values\" FROM forms_valuehelp.forms_vh_values WHERE id=? AND locale=? LIMIT 1",
                (rs, rowNum) -> {
                    try {
