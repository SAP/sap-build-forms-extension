sap.ui.define(["sap/ui/core/mvc/Controller"], function (BaseController) {
  ...
  return BaseController.extend(
    "forms.sbpa.taskui.workflowuimodule.controller.App",
    {
      onInit() {
        ...
        contextModel.dataLoaded().then(function () {
          ...
          if (formsScenarioBaseUrl && taskInstanceId) {
            url = `${formsScenarioBaseUrl}/task/${taskInstanceId}?embedded=sbpaInbox`;
          }
          if (url) {
            //            var formsIframeContainer = theView.byId("formsIframeContainer");
            var oDynamicContentBox = theView.byId("dynamicContentBox");
            var embedTask = false;

            if (embedTask) {
              var oHTML = new HTML({
                content: '<iframe id="taskIframe" width="100%" height="100%" src="${url}" frameborder="0"></iframe>'
              });
              oDynamicContentBox.addItem(oHTML);
              //              formsIframeContainer.setContent(
              //                `<iframe id="taskIframe" width="100%" height="100%" src="${url}" frameborder="0"></iframe>`
              //              );
              ...
              if (sectionElement) {
                ...
                const observer = new MutationObserver((mutations) => {
                  mutations.forEach((mutation) => {
                    ...
                    if (isConsoleLog) console.log("Mutation detected!", mutation);
      ...
      searchSectionRecursivelyAboveFromRefElement: function (referenceElement) {
