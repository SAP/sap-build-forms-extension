...
public abstract class BaseHanaLogEntryDao implements LogEntryDao {
     ...
     */
    protected BaseHanaLogEntryDao(final DataSource ds) {
    ...
    public void insert(LogEntry logEntryEntity) {
        ...
        jdbc.update(con -> {
            final var ps = con.prepareStatement(
                    "INSERT INTO forms_data.forms_log_entries " +
    ...
    public List<LogEntry> getByFormId(String formId) {
        return jdbc.query(con -> {
            final var ps = con.prepareStatement("SELECT * FROM forms_data.forms_logs WHERE form_id = ? ORDER BY ts DESC");
